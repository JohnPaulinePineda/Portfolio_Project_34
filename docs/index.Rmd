...
|
|
| [Least Absolute Shrinkage and Selection Operator Regression](https://rss.onlinelibrary.wiley.com/doi/abs/10.1111/j.2517-6161.1996.tb02080.x) is a regression technique aimed at preventing overfitting when the model is too complex and fits the training data very closely, but performs poorly on new and unseen data. The algorithm adds a penalty term (referred to as the L1 regularization) to the regression cost function that is proportional to the absolute value of the coefficients. This approach can be useful for feature selection, as it tends to shrink the coefficients of less important predictor variables to zero, which can help simplify the model and improve its interpretability. A hyperparameter fraction that controls the ratio of the L1 norm of the coefficient vector relative to the norm at the full solution needs to be optimized through cross-validation.
|
|
...
LASSO_Train_IV_Summary <- data.frame(matrix(ncol=4, nrow=8))
names(LASSO_Train_IV_Summary) <- c("Algorithm","Validation","MedianAUROC","Time")
...
LASSO_Train_EV_Summary <- data.frame(matrix(ncol=4, nrow=8))
names(LASSO_Train_EV_Summary) <- c("Algorithm","Validation","MedianAUROC","Time")
...
ENET_Train_IV_Summary <- data.frame(matrix(ncol=4, nrow=8))
names(ENET_Train_IV_Summary) <- c("Algorithm","Validation","MedianAUROC","Time")
...
ENET_Train_EV_Summary <- data.frame(matrix(ncol=4, nrow=8))
names(ENET_Train_EV_Summary) <- c("Algorithm","Validation","MedianAUROC","Time")
...
MCP_Train_IV_Summary <- data.frame(matrix(ncol=4, nrow=8))
names(MCP_Train_IV_Summary) <- c("Algorithm","Validation","MedianAUROC","Time")
...
MCP_Train_EV_Summary <- data.frame(matrix(ncol=4, nrow=8))
names(MCP_Train_EV_Summary) <- c("Algorithm","Validation","MedianAUROC","Time")
...
SCAD_Train_IV_Summary <- data.frame(matrix(ncol=4, nrow=8))
names(SCAD_Train_IV_Summary) <- c("Algorithm","Validation","MedianAUROC","Time")
...
SCAD_Train_EV_Summary <- data.frame(matrix(ncol=4, nrow=8))
names(SCAD_Train_EV_Summary) <- c("Algorithm","Validation","MedianAUROC","Time")
...
FLASSO_Train_IV_Summary <- data.frame(matrix(ncol=4, nrow=8))
names(FLASSO_Train_IV_Summary) <- c("Algorithm","Validation","MedianAUROC","Time")
...
FLASSO_Train_EV_Summary <- data.frame(matrix(ncol=4, nrow=8))
names(FLASSO_Train_EV_Summary) <- c("Algorithm","Validation","MedianAUROC","Time")
